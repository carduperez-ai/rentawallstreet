...
from src.accounting.currency_valuation import get_eur_per_usd, oracle
...
class BinanceIngestor:
        ...
        try:
            ...
            if ext in ('.xlsx', '.xls'):
                ...
                for i in range(min(50, len(df_raw))):
                    ...
                    if sum(1 for val in row_vals if any(k in val for k in ['tiempo', 'time', 'fecha', 'date', 'moneda', 'asset', 'coin', 'cambio', 'change'])) >= 2:
                        header_idx = i
                        break
                ...
                df_raw = pd.read_csv(file_path, sep=None, engine='python', header=None, encoding='utf-8-sig', on_bad_lines='skip')
                for i in range(min(20, len(df_raw))):
                    row_vals = [str(x).lower() for x in df_raw.iloc[i].values if pd.notna(x)]
                    if sum(1 for val in row_vals if any(k in val for k in ['tiempo', 'time', 'fecha', 'date', 'moneda', 'asset', 'coin', 'cambio', 'change'])) >= 2:
                        header_idx = i
                        break
                df = pd.read_csv(file_path, sep=None, engine='python', header=header_idx, encoding='utf-8-sig', on_bad_lines='skip')
                ...
                with pdfplumber.open(file_path) as pdf:
                    for page in pdf.pages:
                        ...
                        for line in text.split('\n'):
                            line = line.strip()
                            if any(h in line.lower() for h in ['id de usuario', 'historial de', 'nombre:', 'correo', 'www.binance', 'page ']):
                                continue
                            parts = re.split(r'  +', line)
                            if len(parts) >= 5 and re.match(r'^\d{7,}$', parts[0]):
                                if re.match(r'^\d{2}-\d{2}-\d{2}$', parts[1]) and len(parts) > 2 and re.match(r'^\d{2}:\d{2}:\d{2}$', parts[2]):
                ...
                if not all_rows:
                    ...
                    return False
                df = pd.DataFrame(all_rows, columns=['id de usuario', 'tiempo', 'cuenta', 'operacion', 'moneda', 'cambio', 'observacion'])
                header_idx = 0 
            ...
            raw_headers = []
            for h in df.columns:
                if pd.isna(h) or str(h).startswith('Unnamed:'): raw_headers.append('empty_col')
                else:
                    clean_h = str(h).strip().lower()
                    clean_h = re.sub(r'[^a-z0-9_]', '', clean_h.replace(' ', '_').replace('ó', 'o').replace('í', 'i').replace('á', 'a'))
            ...
            self.processed_files.append({
                "name": filename, "platform": platform,
                "tx_count": len(data_df)
            })
            return True
    ...
    def transactions(self) -> List[Trade]:
        ...
        for df in self.spot_dfs:
            ...
            for key in ['total', 'importe', 'cantidad']:
                ...
                if idx_tot_col is not None: break
            ...
            for _, row in df.iterrows():
                ...
                mirror_table.append(Trade(
                    date=dt, asset=self._extract_asset(pair), platform="Binance",
                    direction='buy' if any(x in str(row[idx_l]).upper() for x in ['BUY', 'COMPRA']) else 'sell',
                    quantity=qty, value_eur=val_eur, fee_eur=fee_eur,
                    asset_type="crypto", notes=f"SPOT | {pair}"
                ))
        ...
        for df in self.trans_dfs:
            ...
            idx_op = next((c for c in df.columns if any(x in c.lower() for x in ['operacion', 'type', 'operation'])), 'operacion')
            idx_c = next((c for c in df.columns if any(x in c.lower() for x in ['moneda', 'asset', 'coin'])), 'moneda')
            idx_ch = next((c for c in df.columns if any(x in c.lower() for x in ['cambio', 'change', 'amount'])), 'cambio')
        ...
        for k, lines in groups.items():
            ...
            for i in lines:
                op_up = i['op']
                if any(x in op_up for x in ['FEE', 'COMISI', 'COMMISSION']) or (i['qty'] < 0 and i['asset'] == 'BNB' and len(net_by_asset) > 2):
            ...
            # D. Generación de Trades
            # VALORACIÓN CRUZADA: Si la salida no tiene valor, usamos el valor de la entrada
            final_trade_val = val_total_out_eur if val_total_out_eur > 0 else val_total_in_eur

            # Ventas (Base Out -> Counter In)
            for asset, qty in base_out.items():
                v = final_trade_val if val_total_in_eur > 0 or val_total_out_eur > 0 else self._safe_convert(qty, asset, ts)
                mirror_table.append(Trade(
                    date=ts, asset=asset, platform="Binance", direction='sell',
                    quantity=qty, value_eur=v, fee_eur=total_fee_eur,
                    asset_type="crypto", notes=f"History | Group: {k}"
                ))
            
            # Compras (Base In <- Counter Out)
            for asset, qty in base_in.items():
                c = final_trade_val if final_trade_val > 0 else self._safe_convert(qty, asset, ts)
                mirror_table.append(Trade(
                    date=ts, asset=asset, platform="Binance", direction='buy',
                    quantity=qty, value_eur=c, fee_eur=total_fee_eur,
                    asset_type="crypto", notes=f"History | Group: {k}"
                ))
    ...
    # =====================================================================
    def _safe_convert(self, amount: Decimal, asset: str, dt: datetime) -> Decimal:
        ...
        try:
            ...
            if result is None or result == 0:
                ...
                msg = f"⚠️ FALLO DE CONEXIÓN: No se pudo obtener el precio de {asset} el {fecha_str}. " \
    ...
    def _clean_val(self, val):
        ...
        try: return Decimal(res) if res else Decimal('0')
    ...
    def _convert_to_eur(self, amount: Decimal, asset: str, dt: datetime) -> Decimal:
        ...
        if asset in {'USDC', 'USDT', 'BUSD', 'FDUSD', 'DAI'}:
            raise ValueError(f"CRÍTICO: El oráculo falló para {asset} el {dt}. No se permiten estimaciones fijas (0.92) por ley.")
    ...
    def _parse_date(self, s: Any) -> datetime:
        ...
        # Fallback final solo si los anteriores fallan, forzando orden de año
        try:
            return pd.to_datetime(s_str, yearfirst=True, errors='coerce').to_pydatetime().replace(tzinfo=None)
        except:
            return datetime.now()
