...
from src.domain.stock_entities import Trade, FIFOLot, TaxEvent
...
class StockTaxEngine:
    def __init__(self, tax_year: int = 2025):
        ...
        self.raw_trades: List[Trade] = []

    @property
    def tax_events(self) -> List[TaxEvent]:
        return [te for te in self._all_tax_events if getattr(te, 'date').year == self.tax_year]
    ...
    def process_trades(self, trades: List[Trade]):
        ...
        reorgs = [t for t in trades if any(kw in t.notes.upper() for kw in ['SPLIT', 'REORG', 'CAMBIO DE PRODUCTO']) or 'split' in t.direction]
        processed_keys = set()
        ...
        for t in sorted(reorgs, key=lambda x: x.date):
            ...
            out_t = [r for r in reorgs if r.date.date() == t.date.date() and r.asset.upper().strip() == t.asset.upper().strip() and (r.direction in ['sell', 'split_out'])]
            in_t = [r for r in reorgs if r.date.date() == t.date.date() and r.asset.upper().strip() == t.asset.upper().strip() and (r.direction in ['buy', 'split_in'])]
            ...
            if out_t and in_t:
                ...
                self.ledger.add_event(CorporateActionEvent(
                    event_date=t.date,
        ...
        for t in sorted(trades, key=lambda x: x.date):
            if any(kw in t.notes.upper() for kw in ['SPLIT', 'REORG', 'MERGER', 'CAMBIO DE PRODUCTO']) or 'split' in t.direction:
                continue

            target_id = (t.isin or t.asset).upper().strip()
            
            if t.direction == "buy":
                lot = FIFOLot(t.date, target_id, t.asset, t.quantity, t.value_eur / t.quantity, t.platform, t.fee_eur / t.quantity, source_buy_id=id(t))
                    ...
                    if (lot.isin or lot.asset).upper().strip() in ancestors and lot.date <= t.date:
                        ...
                        if p_lot.current_asset_id == target_id:
                            ...
                            if orig_avail > EPSILON:
                                p_lot.projected_quantity = p_lot.projected_quantity * (Decimal(str(orig_avail)) / Decimal(str(lot.quantity)))
                ...
                for p in projected_inventory:
                    ...
                    if p.projected_quantity <= remaining + EPSILON:
                        ...
                        q_eff = remaining
                        q_orig = (remaining / p.projected_quantity) * (lot.quantity - lot.consumed_original_qty) if p.projected_quantity > 0 else Decimal('0')
                    ...
                    self._all_tax_events.append(TaxEvent(
                        date=t.date, platform=t.platform, asset=t.asset, isin=target_id, ticker=t.ticker,
                        asset_type=t.asset_type or "stock", quantity_sold=q_eff, acquisition_date=p.date,
                        acquisition_cost_eur=c_orig, sale_proceeds_eur=s_proc,
                        buy_fee_eur=f_orig, sell_fee_eur=s_fee,
                        gain_loss_eur=(s_proc - s_fee) - (c_orig + f_orig), notes=t.notes,
                ...
                if remaining > EPSILON:
                    self.warnings.append(f"⚠️ ERROR DE TRAZABILIDAD: Faltan {remaining} acciones en el historial para {t.asset} ({target_id}) en fecha {t.date.date()}")
                ...
                lot = FIFOLot(t.date, target_id, t.asset, t.quantity, cpu, t.platform, Decimal('0'), source_buy_id=id(t))
                lot.consumed_original_qty = Decimal('0')
                self.all_static_lots.append(lot)
    ...
    def ingest_degiro(self, file_paths: List[str]):
        ...
        for div in d:
            if div.date.year == self.tax_year: self.dividends.append(div)
    ...
    def apply_wash_sale_rules(self):
        if not self._all_tax_events: return
        ...
        for te in self._all_tax_events:
            ...
            if s_id:
                if s_id not in sales_by_buy_id: sales_by_buy_id[s_id] = []
            ...
            if getattr(te, 'is_wash_sale', False) and hasattr(te, 'blocked_by_buys'):
                ...
                if consuming_sales and total_repurchased_qty > Decimal('0'):
                    ...
                    for c_sale in consuming_sales:
                        ...
                        buy_origin = next((b for b in getattr(te, 'blocked_by_buys', []) if id(b) == getattr(c_sale, 'source_buy_id', 0)), None)
                        if not buy_origin: continue
                        
                        cost_of_buy = buy_origin.value_eur
                        if cost_of_buy <= EPSILON:
                            ratio = getattr(c_sale, 'quantity_sold') / buy_origin.quantity if buy_origin.quantity > 0 else Decimal('0')
                        else:
                            ratio = getattr(c_sale, 'acquisition_cost_eur') / cost_of_buy
                        ...
                        if global_ratio > Decimal('0.001'):
                            notes_val = te.notes + " [DESBLOQUEADA en el año]" if c_sale.date.year == te.date.year else te.notes + f" [AFLORADA: Desbloqueo interanual por venta en {c_sale.date.year}]"
                            date_val = te.date if c_sale.date.year == te.date.year else datetime(c_sale.date.year, 12, 31, 23, 59, 59)
                            ...
                            unblock_te = dataclasses.replace(
                                ...
                                gain_loss_eur_effective=getattr(te, 'gain_loss_eur_effective', te.gain_loss_eur) * global_ratio,
                                is_wash_sale=False,
                                notes=notes_val,
                                date=date_val
                            )
                            ...
                            remaining_te = dataclasses.replace(
                                ...
                                acquisition_cost_eur=remaining_te.acquisition_cost_eur - unblock_te.acquisition_cost_eur,
                    ...
                    if remaining_te.quantity_sold > EPSILON:
                        remaining_te = dataclasses.replace(
                            remaining_te,
                            notes=remaining_te.notes.split("[DESBLOQUEO")[0] + " [REMANENTE BLOQUEADO]"
                        )
