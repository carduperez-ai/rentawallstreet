...
class WashSaleScanner:
    ...
    def scan(self, tax_events, days_limit=60):
        ...
        for t in self.trades:
            ...
            if key not in assets_map: assets_map[key] = []
        ...
        for te in tax_events:
            ...
            # Ampliar a 365 días si es no cotizada (no hay flag aún, por lo que asume cotizada 60 días)
            limit_days = 365 if getattr(te, 'is_unlisted', False) else days_limit
            start_limit = te.date - timedelta(days=limit_days)
            end_limit = te.date + timedelta(days=limit_days)
            ...
            te = dataclasses.replace(
                ...
                notes=f"Wash Sale: {blocked_ratio*100:.1f}% de la pérdida bloqueada por recompra en +/- {limit_days} días."
            )
            new_events.append(te)
