    ...
    def consume_fifo(self, quantity: Decimal) -> Tuple[Decimal, Decimal, List[dict]]:
        ...
        while remaining > Decimal('0') and self.inventory:
            ...
            if lot.quantity <= remaining + EPSILON:
                ...
                matches.append({'qty': qty, 'cost': lot.quantity * lot.cost_per_unit_eur, 'fee': lot.quantity * lot.buy_fee_per_unit_eur, 'date': lot.date, 'source_buy_id': getattr(lot, 'source_buy_id', 0)})
                ...
                matches.append({'qty': qty, 'cost': qty * lot.cost_per_unit_eur, 'fee': qty * lot.buy_fee_per_unit_eur, 'date': lot.date, 'source_buy_id': getattr(lot, 'source_buy_id', 0)})
                self.matched_qty += qty
                lot.quantity -= qty
                remaining = Decimal('0')
