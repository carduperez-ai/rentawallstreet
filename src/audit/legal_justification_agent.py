...
class LegalJustificationAgent:
    ...
    def analyze_and_justify(self, crypto_events: List[TaxEvent], all_trades: List):
        ...
        for te in crypto_events:
            ...
            if te.gain_loss_eur < 0:
                ...
                rebuys = [
                    b
                    for b in buys_map.get(te.asset.upper(), [])
                    if start <= b.date <= end and b.date != te.acquisition_date
                ]

                if rebuys:
                    self.audit_findings.append(
                        {"event": te, "risk": risk, "observation": observation, "rebuys_detected": len(rebuys)}
                    )
    ...
    def _evaluate_risk(self, event: TaxEvent) -> tuple:
        ...
        if asset in ["USDT", "USDC", "DAI", "BUSD", "EURC"]:
            return (
                "MEDIUM",
                "Stablecoin. Aunque no es valor homogéneo, su naturaleza fiat-peg podría ser objeto de analogía con divisas por la AEAT.",
            )
        ...
        if any(x in asset for x in ["ETP", "ETN", "FUTURE", "PERP", "OPTION"]):
            return (
                "HIGH",
                "Instrumento derivado o cotizado. Alta probabilidad de ser clasificado como valor homogéneo (Art. 33.5 f LIRPF).",
            )
    ...
    def _generate_forensic_report(self):
        ...
        with open(self.output_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                "Este informe evalúa de forma autónoma la deducibilidad de las pérdidas patrimoniales generadas en activos digitales, "
            ...
            if not self.audit_findings:
                f.write(
                    "✅ **Análisis completado:** No se han detectado operaciones que requieran justificación especial o supongan un riesgo de colisión normativa.\n"
                )
            else:
                for find in self.audit_findings:
                    ...
                    f.write(
                        f"- **Operación:** Venta el {te.date.strftime('%d/%m/%Y')} con pérdida de {abs(te.gain_loss_eur):.2f} €\n"
                    )
                    f.write(
                        f"- **Conflicto detectado:** Recompra en ventana de 60 días ({find['rebuys_detected']} ops).\n"
                    )
                    f.write(f"- **Dictamen del Agente:** {find['observation']}\n")
                    if find["risk"] == "HIGH":
                        f.write(
                            "> ⚠️ **ADVERTENCIA:** Se recomienda precaución en la deducibilidad de esta pérdida específica.\n"
                        )
                    f.write("\n")

            f.write("\n## 3. CONCLUSIÓN TÉCNICA\n")
            f.write(
                "La aplicación de la base imponible del ahorro en este ejercicio se considera, con carácter general, alineada "
