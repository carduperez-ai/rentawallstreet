...
def parse(file_path: str) -> Tuple[List[Trade], List[Dividend]]:
    ...
    if ext in ['.csv', '.xlsx', '.xls']: return _parse_csv_excel(file_path, ext)
    elif ext == '.pdf': return _parse_pdf(file_path)
    return [], []
...
def _parse_csv_excel(file_path: str, ext: str) -> Tuple[List[Trade], List[Dividend]]:
    ...
    for _, row in df.iterrows():
        ...
        if any(kw in action for kw in ['buy', 'sell', 'rights', 'split']):
            ...
            if 'split' in action: direction = "split_in" if 'open' in action else "split_out"
            ...
            final_trades.append(Trade(date=dt, platform="Trading 212", asset=name or ticker, isin=isin, ticker=ticker, asset_type="stock", direction=direction, quantity=abs(qty), value_eur=total_bruto, fee_eur=fee, notes="T212_CSV"))
        elif any(kw in action for kw in ['dividend', 'interest']):
            wht, net = abs(clean_decimal(row[c_withholding])) if c_withholding and pd.notna(row[c_withholding]) else Decimal('0'), abs(clean_decimal(row[c_total]))
            final_divs.append(Dividend(date=dt, platform="Trading 212", asset=name or "INTERESES/DIVIDENDOS T212", isin=isin, gross_eur=net + wht, withholding_foreign_eur=wht, withholding_spain_eur=Decimal('0'), type='interest' if 'interest' in action else 'dividend'))
            ...
            for page in pdf.pages:
                ...
                for line in lines:
                    ...
                    if m_sale and any(x in text.lower() for x in ['venta', 'posici.n cerrada', 'posicion cerrada']):
                        ...
                        if len(nums) >= 4:
                            trades.append(Trade(date=datetime.strptime(m_sale.group(1), "%d.%m.%Y"), platform="Trading 212", asset=m_sale.group(2), isin=m_sale.group(3), ticker=m_sale.group(2), asset_type="stock", direction="sell", quantity=_pdf_to_decimal(nums[0]), value_eur=_pdf_to_decimal(nums[-2]), fee_eur=Decimal('0'), notes="T212_PDF_SALE"))
                        continue

                    # 2. Intentar capturar DIVIDENDOS
                    m_div = re.search(r"([A-Za-z0-9\s\.-]+)\s+([A-Z]{2}[0-9A-Z]{5,15})\s+[A-Z]{2}\s+[\d\.,]+\s+(\d{2}\.\d{2}\.\d{4})", line)
                    if m_div and 'dividendo' in text.lower():
                        ...
                        if len(nums) >= 4:
                            ...
                            dividends.append(Dividend(date=datetime.strptime(m_div.group(3), "%d.%m.%Y"), platform="Trading 212", asset=m_div.group(1).strip(), isin=isin, gross_eur=gross, withholding_foreign_eur=Decimal('0') if is_spain else wht, withholding_spain_eur=wht if is_spain else Decimal('0'), type='dividend'))
                        continue
                ...
                if 'inter.s de efectivo' in text.lower():
                    ...
                    if m_int:
                        ...
                        if not any(d.asset == "Intereses Efectivo" and d.gross_eur == _pdf_to_decimal(m_int.group(2)) for d in dividends):
                            dividends.append(Dividend(date=datetime(2025, 12, 31), platform="Trading 212", asset="Intereses Efectivo", isin="", gross_eur=_pdf_to_decimal(m_int.group(2)), withholding_foreign_eur=Decimal('0'), withholding_spain_eur=Decimal('0'), type='interest'))
