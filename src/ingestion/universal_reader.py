class NeedsReviewException(Exception): pass
class UniversalReader:
    @staticmethod
    def parse(f): return parse(f)
import pandas as pd
...
def parse(file_path: str) -> Tuple[List[Trade], List[Dividend]]:
    ...
    for _, row in df.iterrows():
        ...
        if any(kw in op_type for kw in ['compra', 'buy', 'adquisici']):
            final_trades.append(Trade(dt, "Universal", asset, "", "", "stock", "buy", qty, val, Decimal('0'), "GENERIC_BUY"))
        
        elif any(kw in op_type for kw in ['venta', 'sell', 'transmisi']):
            final_trades.append(Trade(dt, "Universal", asset, "", "", "stock", "sell", qty, val, Decimal('0'), "GENERIC_SELL"))
            ...
            final_trades.append(Trade(dt, "Universal", asset, "", "", "stock", "split_in" if clean_decimal(row[c_qty]) > 0 else "split_out", qty, Decimal('0'), Decimal('0'), "GENERIC_SPLIT"))
