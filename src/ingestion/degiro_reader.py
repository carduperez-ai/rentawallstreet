...
def _parse_date(s) -> datetime:
    ...
    if not s or s.lower() == 'nan': return datetime(1900, 1, 1)
...
def _extract_data_from_pdf(path: str) -> Tuple[List[Trade], List[Dividend]]:
    ...
    try:
        ...
        for line in raw_text.split('\n'):
            if re.match(r"^\d{2}-\d{2}-\d{4}", line.strip()): text += "\n" + line.strip()
            else: text += " " + line.strip()

        for line in text.split('\n'):
            line = line.strip()
            if "flatexDEGIRO" in line: line = line.split("flatexDEGIRO")[0].strip()
            match_dt = re.match(r"^(\d{2}-\d{2}-\d{4}\s+\d{2}:\d{2})", line)
            if not match_dt: continue
            match_isin = re.search(r"([A-Z]{2}[A-Z0-9]{9}\d)", line)
            if not match_isin: continue
            dt = _parse_date(match_dt.group(1)); isin = match_isin.group(1)
            prod = line[len(match_dt.group(1)):line.find(isin)].strip().upper()
            parts = line[line.find(isin)+len(isin):].split()
            if len(parts) < 3: continue
            origin = "DESC"; total_eur = Decimal('0')
            for i in range(len(parts)-1, -1, -1):
                p = parts[i]
                if re.match(r"^[A-Z]{3,5}$", p) and not any(c.isdigit() for c in p): origin = p.upper()
                else:
                    try:
            ...
            for i in range(len(parts)):
                ...
                try:
                    ...
                    if re.match(r"^-?[\d\.]+$", p_clean) and p_clean != "0.00":
                        qty = _clean_decimal(p); break
                except: continue
            if qty != 0:
                trades.append(Trade(dt, "DeGiro", prod, isin, "", "stock", "buy" if qty > 0 else "sell", abs(qty), total_eur, Decimal('0'), f"TRANS_FILE|PDF|{origin}"))
...
def parse(file_paths) -> Tuple[List[Trade], List[Dividend]]:
            ...
            if df is not None:
                try:
                    ...
                    for _, group in df.groupby('tmp_group_key', sort=False):
                        ...
                        if not dt: continue  # Sin filtro de año: el motor FIFO necesita historial completo
                        
                        oid = str(first.iloc[16]) if len(first) > 16 else "nan"
                        if oid != "nan" and oid in seen_transactions: continue
                        if oid != "nan": seen_transactions.add(oid)
                        ...
                        t_fee = sum([abs(_clean_decimal(r.iloc[14])) for _, r in group.iterrows()]) if len(first) > 14 else Decimal('0')
                        
                        if t_qty != 0 or t_val != 0:
                            final_trades.append(Trade(
                                date=dt, platform="DeGiro", 
                                asset=str(first.iloc[2]).upper(), 
                                isin=str(first.iloc[3]).upper(), 
                                ticker="", asset_type="stock", 
                                direction="buy" if t_qty > 0 else "sell", 
                                quantity=abs(t_qty), value_eur=abs(t_val), 
                                fee_eur=t_fee, notes=f"TRANS_FILE|ORDER|{oid}",
            ...
            if df is not None:
                ...
                for _, row in df.iterrows():
                    try:
                        ...
                        if sig in seen_account_lines: continue
                        seen_account_lines.add(sig)

                        if any(x in desc for x in ["COMPRA", "VENTA", "TRANSACCIÓN"]): continue
                        
                        desc_upper = desc.upper()
                        if "DIVIDENDO" in desc_upper or "DIVIDEND" in desc_upper:
                            key = (dt.strftime('%Y-%m-%d'), asset)
                            div_map[key]['asset'] = asset; div_map[key]['date'] = dt
                            if isin: div_map[key]['isin'] = isin
                            if "RETENCI" in desc_upper: div_map[key]['withholding'] += val_clean
                            else: div_map[key]['gross'] += val_clean
                            ...
                            if v_clean < 0: 
                                ...
                                final_divs.append(Dividend(dt, "DeGiro", desc, isin, v_clean, Decimal('0'), Decimal('0'), type='interest'))
                            ...
                            final_divs.append(Dividend(dt, "DeGiro", desc, "", Decimal('0'), Decimal('0'), Decimal('0'), type='fee', custody_fee_eur=abs(_clean_decimal(row.iloc[8]))))
                            ...
                            if v_fee > 0:
                                final_divs.append(Dividend(dt, "DeGiro", desc, "", Decimal('0'), Decimal('0'), Decimal('0'), type='fee', custody_fee_eur=v_fee))
                    except Exception as e:
    ...
    for k, v in div_map.items():
        if v['gross'] > 0 or v['withholding'] > 0:
            ...
            final_divs.append(Dividend(v['date'], "DeGiro", v['asset'], isin_final, gross, withh if not is_spain else 0, withh if is_spain else 0, type='dividend'))
    
    # Asegurar que fees_list use valores positivos
    for f in fees_list:
        final_divs.append(Dividend(f.date, f.platform, f.asset, f.isin, f.gross_eur, f.withholding_foreign_eur, f.withholding_spain_eur, type=f.type, custody_fee_eur=abs(f.custody_fee_eur)))
