...
CASH_INCOME   = {'dividends', 'payment in lieu of dividends',
...
class IBKRIngestor:
    ...
    def process_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            section_markers = {'trades', 'cashtransactions', 'cash transactions',
                                'openpositions', 'statement'}
    ...
    def transactions(self) -> List[StockTrade]:
        ...
        for row in self._trade_rows:
            ...
            commissions = abs(_dec(g('Commissions', 'commissions',
            ...
            if trade_type == 'corpact' and proceeds == Decimal('0'):
                ...
                trades.append(StockTrade(
                    date=dt, platform='IBKR', asset=symbol,
                    isin=isin, ticker=symbol,
                    asset_type='stock', direction=direction,
                    quantity=qty, value_eur=Decimal('0'), fee_eur=Decimal('0'),
            ...
            trades.append(StockTrade(
                date=dt, platform='IBKR', asset=symbol,
                isin=isin, ticker=symbol,
                asset_type='stock', direction='buy' if buy_sell == 'BUY' else 'sell',
            ...
            if cash_type in CASH_INCOME:
                ...
                if key in pending:
                    ...
                    pending[key] = StockDividend(
                        date=existing.date, platform='IBKR',
                        asset=existing.asset, isin=existing.isin,
                    ...
                    pending[key] = StockDividend(
                        date=dt, platform='IBKR', asset=symbol, isin=isin,
                        gross_eur=val_eur, withholding_foreign_eur=Decimal('0'),
                        withholding_spain_eur=Decimal('0'), type=income_type,
                    )
                ...
                if key in pending:
                    ...
                    pending[key] = StockDividend(
                        date=dt, platform='IBKR', asset=symbol, isin=isin,
                        gross_eur=Decimal('0'), withholding_foreign_eur=val_eur,
