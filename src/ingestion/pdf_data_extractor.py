    ...
    def process(self, filepath: str) -> Optional[Dict]:
        try:
            with pdfplumber.open(filepath) as pdf:
                ...
                if m1: summary["comisiones_posiciones_cerradas"] = self._to_decimal(m1.group(1))

                # 2. Total comisiones pagadas (Nota *) - Usamos comodín para 'año'
                m2 = re.search(r"pagadas en el a.*?o 2025 \*?\s+([\d\.,]+)", full_text)
                if m2: summary["comisiones_pagadas_2025"] = self._to_decimal(m2.group(1))

                # 3. Tobin e ITF
                m3 = re.search(r"Transacciones Financieras realizado\s+([\d\.,]+)", full_text)
                if m3: summary["tasa_tobin_realizada"] = self._to_decimal(m3.group(1))

                # 4. Conectividad
                m4 = re.search(r"conectividad con el mercado\s+([\d\.,]+)", full_text)
                if m4: summary["conectividad_mercado"] = self._to_decimal(m4.group(1))

                # 5. Ganancias y Pérdidas (NUEVO)
                m_gain = re.search(r"Ganancias patrimoniales totales\s+([\d\.,]+)", full_text)
                if m_gain: summary["ganancias_patrimoniales"] = self._to_decimal(m_gain.group(1))
                
                m_loss = re.search(r"P[eé]rdidas totales\s+([\d\.,]+)", full_text)
                if m_loss: summary["perdidas_totales"] = self._to_decimal(m_loss.group(1))
                ...
                div_section = re.search(r"Dividendos, Cupones y otras remuneraciones(.*?)(?:Distribuciones|Relaci[oó]n de ganancias)", full_text, re.DOTALL)
                if div_section:
                ...
                if "**" in full_text: summary["notas"].append("Nota ** detectada: Incluye costes históricos de apertura.")
                if "*" in full_text: summary["notas"].append("Nota * detectada: Incluye costes de cambio de divisa manual.")
            ...
            with pdfplumber.open(filepath) as pdf:
                for page in pdf.pages:
                    ...
                    for line in lines:
                        ...
                        match = re.search(r"(\d{2}-\d{2}-\d{4})\s+(\d{2}:\d{2}).*?([A-Z0-9]{12}).*?\s+(-?\d+)\s+([\d,.]+)\s+[A-Z]{3}.*?([-?\d,.]+)\s+([-?\d,.]+)\s+([-?\d,.]+)\s*$", line)
                        if match:
                            ...
                            trades.append(Trade(
                                date=dt, platform="DeGiro", asset=isin, isin=isin, 
                                ticker="", asset_type="stock",
                                direction="buy" if q > 0 else "sell",
                                quantity=abs(q), value_eur=abs(eur_val),
