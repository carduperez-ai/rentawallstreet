...
IGNORED_TYPES = {
    ...
    'margin', 'settled', 'credit', 'custodytransfer',
    'nftcreatorfee', 'nftrebate',
}
FEE_TYPES = {'rollover'}
        ...
        for refid, legs in groups.items():
            ...
            if not sold_legs or not bought_legs:
                ...
                for r in legs:
                    ...
                    trades.append(Trade(
                        date=dt, asset=asset, platform='Kraken',
                        direction=direction, quantity=abs(amt),
            ...
            if sold_asset not in FIAT:
                trades.append(Trade(
                    date=dt, asset=sold_asset, platform='Kraken',
                    direction='sell', quantity=sold_qty,
                    value_eur=val_eur, fee_eur=fee_eur,
                    asset_type='crypto', notes=notes,
                ))

            # COMPRA del activo entrante (si no es fiat)
            if bought_asset not in FIAT:
                trades.append(Trade(
                    date=dt, asset=bought_asset, platform='Kraken',
                    direction='buy', quantity=bought_qty,
                    value_eur=val_eur, fee_eur=fee_eur,
                    asset_type='crypto', notes=notes,
                ))
        ...
        for row in self._rows:
            ...
            if tx_type in FEE_TYPES:
                result.append(Dividend(
                    date=dt, platform='Kraken', asset=asset, isin='',
                    gross_eur=Decimal('0'),
