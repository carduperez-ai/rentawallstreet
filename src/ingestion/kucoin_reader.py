...
class KuCoinIngestor:
        ...
        try:
            ...
            if fmt == 'spot':
                ...
                self.warnings.append(
                    f"Formato KuCoin no reconocido en {filename}. "
                    f"Cabeceras: {list(rows[0].keys())}"
                )
                return False
    ...
    def transactions(self) -> List[Trade]:
        ...
        for row in self._spot_rows:
            ...
            trades.append(Trade(
                date=dt, asset=base, platform='KuCoin',
                direction='buy' if 'buy' in side else 'sell',
                quantity=amount, value_eur=val_eur, fee_eur=fee_eur,
                asset_type='crypto',
            ...
            if tx_type in FUNDING_FEE:
                ...
                if raw_amt < Decimal('0') or 'interest' in tx_type:
                    result.append(Dividend(
                        date=dt, platform='KuCoin', asset=coin, isin='',
