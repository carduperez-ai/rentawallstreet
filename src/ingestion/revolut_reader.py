    ...
    for _, row in df.iterrows():
        ...
        if any(kw in t_type for kw in ['buy', 'sell']):
            ...
            final_trades.append(Trade(
                date=dt, platform="Revolut", asset=ticker, isin="", ticker=ticker,
                asset_type="stock", direction="buy" if "buy" in t_type else "sell",
                quantity=qty, value_eur=val_eur, fee_eur=fee_eur, notes=f"REVOLUT_{t_type} | Rate: {rate}"
            ))
        ...
        # 3. DIVIDENDOS
            ...
            final_divs.append(Dividend(
                date=dt, platform="Revolut", asset=ticker, isin="",
                gross_eur=div_eur, withholding_foreign_eur=Decimal('0'),
        ...
        # 3. SPLITS
            ...
            final_trades.append(Trade(
                date=dt, platform="Revolut", asset=ticker, isin="", ticker=ticker,
                asset_type="stock", direction="split_in" if qty > 0 else "split_out",
                quantity=abs(qty), value_eur=Decimal('0'), fee_eur=Decimal('0'), notes="REVOLUT_SPLIT"
            ))
