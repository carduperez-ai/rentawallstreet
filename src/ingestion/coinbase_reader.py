...
def _safe_eur(
    amount: Decimal, asset: str, dt: datetime, spot_price: Decimal = None, spot_currency: str = None
) -> Decimal:
    """
...
class CoinbaseIngestor:
    ...
    def process_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            for raw_row in raw_rows:
                norm = {col_map[k]: v.strip() if isinstance(v, str) else v for k, v in raw_row.items()}
    ...
    def transactions(self) -> List[Trade]:
        ...
        for row in self._rows:
            ...
            val_eur = (
                _safe_eur(val_source, spot_currency or "USD", dt)
                if spot_currency in FIAT or spot_currency == ""
                else _safe_eur(qty, asset, dt, spot_price, spot_currency)
            )
            ...
            for i in range(len(legs)):
                ...
                if best_match is not None:
                    ...
                    for leg in (leg1, leg2):
                        ...
                        trades.append(
                            Trade(
                                ...
                                direction=direction,
                                quantity=leg["qty"],
                                value_eur=leg["val_eur"],
                                fee_eur=leg_fee,  # val individual por pierna
                                asset_type="crypto",
                    ...
                    self.warnings.append(
                        f"Fallo al emparejar Convert: {legs[i]['asset']} ({legs[i]['dt']}). Posible orfandad."
                    )
