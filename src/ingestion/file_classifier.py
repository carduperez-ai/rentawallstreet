...
class FileClassifier:
    ...
    PLATFORMS = {
        'BINANCE': ['User_Id', 'Time', 'Account', 'Operation', 'Coin', 'Hora_UTC', 'Operacion', 'Moneda', 'Market', 'Pair', 'Side', 'Fee'],
    ...
    def classify(cls, file_path: str, user_pref: str = "auto") -> FileMetadata:
        ...
        if platform == "UNKNOWN":
            if 'BINANCE' in filename: platform = 'BINANCE'
            elif 'DEGIRO' in filename: platform = 'DEGIRO'
            elif 'TRADING212' in filename or 'T212' in filename or ('TRADING' in filename and 'BINANCE' not in filename): platform = 'TRADING212'
            elif 'REVOLUT' in filename: platform = 'REVOLUT'
            elif 'IBKR' in filename or 'IB ' in filename or 'INTERACTIVE' in filename: platform = 'IBKR'
            elif 'ETORO' in filename: platform = 'ETORO'
            elif 'COINBASE' in filename: platform = 'COINBASE'
            elif 'KRAKEN' in filename: platform = 'KRAKEN'
            elif 'KUCOIN' in filename: platform = 'KUCOIN'
        ...
        if 'ORDEN' in filename or 'ORDER' in filename or 'SPOT' in filename:
            ...
            content_type = 'CUENTA'
        elif 'REPORT' in filename or 'INFORME' in filename or 'ANNUAL' in filename or 'RESUMEN' in filename or 'STATEMENT' in filename:
