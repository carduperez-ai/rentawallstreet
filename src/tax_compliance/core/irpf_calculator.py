...
from src.domain.fiscal_entities import Dividend, WorkIncome, TaxpayerProfile, RentalIncome, OtherIncome, IncomeSubtype, FamilyRole, LossCarryForward, Deduction
...
class IRPFCalculator:
    ...
    def __init__(
        self,
        crypto_events: List = None,
        stock_events: List = None,
        dividends: List[Dividend] = None,
        work: Optional[WorkIncome] = None,
        rental_income: List[RentalIncome] = None,
        other_pyg: List[OtherIncome] = None,
        business_income: List = None,
        attributed_income: List = None,
        ...
        self.crypto_events = crypto_events or []
        self.stock_events = stock_events or []
        self.events = self.crypto_events + self.stock_events
        self.dividends = dividends or []
        self.work = work
        self.rental_income = rental_income or []
        self.other_pyg = other_pyg or []
        self.business_income = business_income if business_income is not None else []
        self.attributed_income = attributed_income if attributed_income is not None else []
        self.region = region
    ...
    def calculate(self) -> Dict:
        ...
        gpp_neto = sum(
            (e.gain_loss_eur for e in self.events
             if e.asset not in fiat_stables and not (getattr(e, 'is_wash_sale', False) and getattr(e, 'asset_type', '') != 'crypto' and e.gain_loss_eur < 0)),
            Decimal('0')
        )
        ...
        for pyg in self.other_pyg:
            ...
            if pyg.category in self._BG_CATEGORIES:
                if pyg.gain_loss_eur >= 0:
                    other_bg_pos += pyg.gain_loss_eur
                else:
                    other_bg_neg += pyg.gain_loss_eur
            else:
                gpp_neto += pyg.gain_loss_eur

        # G/P solo de cripto
        crypto_gain_loss = sum(
            (e.gain_loss_eur for e in self.crypto_events
             if e.asset not in fiat_stables),
            Decimal('0')
        ) + stables_vigilante_gpp
        # G/P solo de acciones/ETF
        stock_gain_loss = sum(
            (e.gain_loss_eur for e in self.stock_events),
            Decimal('0')
        )

        # 2. RCM bruto (dividendos + intereses + staking)
        rcm_bruto = sum((d.gross_eur for d in self.dividends), Decimal('0'))
        ...
        custody_fees_total = sum(
            (getattr(d, 'custody_fee_eur', Decimal('0')) for d in self.dividends
        ...
        for r in self.rental_income:
            ...
            if tipo == "arrendado":
                ...
                gastos_grupo_a = getattr(r, 'expenses_maintenance', Decimal('0')) + getattr(r, 'expenses_mortgage_interest', Decimal('0'))
                ...
                if r.reduction_habitual and neto > 0:
                    if getattr(r, 'contract_year', 2023) < 2024:  # Aprox. DT 37ª LIRPF (corte real: 26/05/2023)
                        pct = Decimal('0.60')
                    elif getattr(r, 'zona_tensionada', False) and getattr(r, 'rebaja_5', False):
                ...
                porcentaje_imputacion = Decimal('0.011') if getattr(r, 'valor_catastral_revisado', False) else Decimal('0.02')
        ...
        # Guardar saldos originales para el límite del 25% (Art. 49.1 a y b)
        import decimal
        limite_cruzado_rcm_total = (max(rcm_final, Decimal('0')) * Decimal('0.25')).quantize(Decimal('0.01'), rounding=decimal.ROUND_HALF_UP)
        limite_cruzado_gpp_total = (max(gpp_final, Decimal('0')) * Decimal('0.25')).quantize(Decimal('0.01'), rounding=decimal.ROUND_HALF_UP)

        # Paso 1: Compensación CRUZADA del EJERCICIO (Art. 49.1)
        ...
        # Paso 2: Compensación HOMÓLOGA de años anteriores (Art. 49.2)
        ...
        if remaining_gpp_cf > 0 and gpp_final > 0:
            ...
            for y, v in cons.items(): self.consumed_gpp_cf[y] = self.consumed_gpp_cf.get(y, Decimal('0')) + v
        ...
        if remaining_rcm_cf > 0 and rcm_final > 0:
            ...
            for y, v in cons.items(): self.consumed_rcm_cf[y] = self.consumed_rcm_cf.get(y, Decimal('0')) + v
        ...
        if self.work:
            ...
            work_bruto_total = max(self.work.retribuciones_dinerarias + species_income - reduccion_irr_work, Decimal('0'))
            ...
            if _is_married_joint:
                ...
                if getattr(self.profile, 'strict_dgt_compliance', False) and getattr(self.profile, 'spouse_income_eur', Decimal('0')) > Decimal('0'):
                    gastos_art19_s2 += Decimal('2000')

                rnt_art20_s2 = max(
                    getattr(self.profile, 'spouse_income_eur', Decimal('0')) - gastos_art19_s2,
                    Decimal('0')
                )
            ...
            if otras_rentas <= Decimal('6500'):
                if rnt_art20 <= Decimal('14852'):
                    ...
                    reduccion_art20 = max(
                        Decimal('7302') - Decimal('1.75') * (rnt_art20 - Decimal('14852')),
                        Decimal('0')
                    )
                elif rnt_art20 <= Decimal('19747.50'):
                    reduccion_art20 = max(
                        Decimal('2364.34') - Decimal('1.14') * (rnt_art20 - Decimal('17673.52')),
                        Decimal('0')
                    )
        ...
        bg_positivo_para_limite = (
            bg + max(rendimiento_alquiler, Decimal('0')) + total_business_net + other_bg_pos
        )
        if other_bg_neg < 0:
        ...
        irr_eco_val = sum(getattr(b, 'rendimientos_irregulares_eur', Decimal('0')) for b in self.business_income) if self.business_income else Decimal('0')
        ...
        reduccion_pensiones = self._compute_pension_reduccion(rendimiento_neto_trabajo, total_business_net, irr_work_val, irr_eco_val, is_married_joint=_is_joint_pension)
        ...
        cuota_min_est  = self._compute_cuota_minimo(minimo_total, bg, base_ahorro,
                                                    self.GENERAL_BRACKETS_ESTATAL,
                                                    self.SAVINGS_BRACKETS_ESTATAL)
        ...
        deducciones_cuota_integra = deducciones_estatales + deducciones_autonomicas  # V1-003: nombre semánticamente correcto
        ...
        ret_div_esp = sum(
            (d.withholding_spain_eur for d in self.dividends if d.asset not in fiat_stables),
            Decimal('0')
        )
        ...
        cuota_resultante = max(
            cuota_liquida - deduccion_doble_imposicion,
            Decimal('0')
        )
        ...
        raw_summary = {
            ...
            "work_pension_empresa": (
                self.profile.pension_empresa_eur if self.profile else Decimal('0')
            ),
            ...
            "gastos_custodia_total": custody_fees_total,
            "rcm_neto_gastos": rcm_neto_gastos,
            "rcm_neto_reducido": max(rcm_final, Decimal('0')),
            "dividendos_brutos": {"val": rcm_bruto},
            "retencion_dividendos": {"val": ret_div_esp},
            "stock_gain_loss": stock_gain_loss,
            "gpp_neto": gpp_final,
            "crypto_gain_loss": crypto_gain_loss,
            "minimo_pf_estatal": minimo_total,
            "minimo_pf_autonomico": minimo_total_auton,
            "cuota_integra_estatal": cuota_integra_estatal,
            "cuota_integra_autonomica": cuota_integra_auton,
            "deducciones_estatales": deducciones_estatales,
            "deducciones_autonomicas": deducciones_autonomicas,
            "deducciones_ccaa": deducciones_cuota_integra,  # key pública intacta para compatibilidad
            "deducciones_diferenciales": deducciones_diferenciales,
            "deduccion_doble_imposicion": deduccion_doble_imposicion,
            "deduccion_rentas_trabajo": deduccion_rentas_trabajo,
            "alimonty_reduccion": alimonty,
            "reduccion_conjunta": reduccion_conjunta,
            "cuota_liquida_estatal": cuota_liquida_estatal,
            "cuota_liquida_autonomica": cuota_liquida_autonomica,
            "cuota_liquida": cuota_liquida,
            "cuota_resultante": cuota_resultante,
            "retenciones_trabajo_total": retenciones_trabajo,
            "retenciones_capital": ret_div_esp,
            "cuota_loterias": cuota_loterias,
            "retenciones_loterias": retenciones_loterias,
            ...
            "wash_sale_events": [
                {"date": e.date, "platform": e.platform, "asset": e.asset,
                 "gain_loss_eur": e.gain_loss_eur, "notes": getattr(e, 'notes', '')}
                for e in self.stock_events if getattr(e, 'is_wash_sale', False)
            ],
        ...
        return TaxResultDTO(
            general_taxable_base=base_imponible_general,
            savings_taxable_base=base_ahorro,
            general_liquidable_base=bg,
            savings_liquidable_base=max(rcm_final, Decimal('0')) + max(gpp_final, Decimal('0')),
            gross_tax_quota=cuota_integra_estatal + cuota_integra_auton,
            net_tax_quota=cuota_liquida,
        ...
        for d in self.dividends:
            ...
            if d_type == 'fee':
                ...
                if plat not in div_details:
                    div_details[plat] = {
                        'gross': Decimal('0'), 'gross_ext': Decimal('0'),
                        'wht_ext': Decimal('0'), 'wht_esp': Decimal('0'),
                        'deduccion_di': Decimal('0'), 'custody_fee': Decimal('0'),
                        'assets': {},
                    }
            ...
            if asset_key not in target[plat]['assets']:
                target[plat]['assets'][asset_key] = {
                    'gross': Decimal('0'), 'wht_ext': Decimal('0'),
                    'wht_esp': Decimal('0'), 'deduccion_di': Decimal('0'),
                }
    ...
    def _group_aeat_events(self, asset_type: str) -> dict:
        ...
        for ev in self.events:
            ...
            if key not in results[plat]:
                results[plat][key] = {
                    "nombre": ev.asset, "isin": isin,
                    "adquisicion": Decimal('0'), "transmision": Decimal('0'),
                    "ganancia": Decimal('0'), "perdida": Decimal('0'),
                }
    ...
    def _compute_minimo_total(self, regional: bool = False, ctx=None) -> Decimal:
        ...
        if self.profile:
            ...
            if (getattr(self.profile, 'disability_needs_help', False)
    ...
    def _compute_cuota_minimo(
        self, minimo: Decimal, bg: Decimal, base_ahorro: Decimal,
        general_brackets: list = None, savings_brackets: list = None,
    ) -> Decimal:
        ...
        return (
            self._apply_brackets(minimo_bg, g_brackets)
            + self._apply_brackets(minimo_ba, s_brackets)
        )

    def _compute_pension_reduccion(self, rendimiento_neto_trabajo: Decimal, total_business_net: Decimal = Decimal('0'), irr_work: Decimal = Decimal('0'), irr_eco: Decimal = Decimal('0'), is_married_joint: bool = False) -> Decimal:
        if not self.profile:
            return Decimal('0')
        ...
        if is_married_joint:
            ...
            if spouse_total_aportado > Decimal('0'):
                spouse_rnt = max(getattr(self.profile, 'spouse_income_eur', Decimal('0')) - getattr(self.profile, 'spouse_ss_eur', Decimal('0')), Decimal('0'))
        ...
        if pension_spouse > Decimal('0'):
            spouse_work_net = getattr(self.profile, 'spouse_income_eur', Decimal('0')) - getattr(self.profile, 'spouse_ss_eur', Decimal('0'))
