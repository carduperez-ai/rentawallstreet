...
class CantabriaDeductor:
    ...
    def calculate(cls, profile: TaxpayerProfile, big: Decimal, bia: Decimal, cuota_auton: Decimal) -> List[Deduction]:
        ...
        if rent_desp > 0:
            ...
            if val > 0:
                deductions.append(Deduction("CTB_ALQ_DESP", val.quantize(Decimal('0.01')), "Alquiler en municipios con riesgo de despoblamiento"))
        ...
        if not applied_alq_desp:
            ...
            if rent_paid > 0 and (age < 36 or age >= 65 or discap >= 65):
                ...
                if rent_paid > bi_total * Decimal('0.10'):
                    ...
                    val = min(pct_val, limit_taxpayer)
                    if val > 0:
                        deductions.append(Deduction("CTB_ALQ", val.quantize(Decimal('0.01')), "Arrendamiento de vivienda habitual general"))
        ...
        if base_limite_renta < Decimal('31485'):
            ...
            if births > 0:
                ...
                if val > 0:
                    deductions.append(Deduction("CTB_NAC", val.quantize(Decimal('0.01')), "Nacimiento o adopción de hijos"))
        ...
        if exp_mej > 0 or prev_unused > 0:
            ...
            if val > 0:
                deductions.append(Deduction("CTB_MEJ", val.quantize(Decimal('0.01')), "Obras de mejora en viviendas de su propiedad"))
        ...
        if don_foundations > 0 or don_coopera > 0 or don_disabled > 0:
            ...
            if val > 0:
                deductions.append(Deduction("CTB_DON", val.quantize(Decimal('0.01')), "Donativos a fundaciones o apoyo a discapacidad"))
        ...
        if foster_count > 0:
            ...
            if val > 0:
                deductions.append(Deduction("CTB_ACOG", val.quantize(Decimal('0.01')), "Acogimiento familiar de menores"))
        ...
        if inv_new > 0:
            ...
            val = min(pct_val, limit_taxpayer)
            if val > 0:
                deductions.append(Deduction("CTB_ACC", val.quantize(Decimal('0.01')), "Inversión en adquisición de acciones de nuevas entidades"))
        ...
        if base_limite_renta < limite_enf:
            ...
            if exp_enf > 0:
                ...
                val = min(pct_val, limit_total)
                if val > 0:
                    deductions.append(Deduction("CTB_ENF", val.quantize(Decimal('0.01')), "Gastos de enfermedad y aparatos médicos"))
        ...
        if guard_desp > 0:
            ...
            if kids > 0:
                ...
                if val > 0:
                    deductions.append(Deduction("CTB_GUARD_DESP", val.quantize(Decimal('0.01')), "Gastos de guardería en municipios con riesgo de despoblamiento"))
        ...
        if not applied_guard_desp:
            ...
            if guard_gen > 0:
                ...
                if kids > 0:
                    ...
                    val = min(pct_val, limit_max) / coprop
                    if val > 0:
                        deductions.append(Deduction("CTB_GUARD", val.quantize(Decimal('0.01')), "Gastos de guardería general"))
        ...
        if profile.regional_data.get('cantabria_relocation_despoblacion_motivos_laborales', False):
            ...
            if year_of_transfer in (2024, 2025):
                ...
                num_contribuyentes = 2 if (es_conjunta and profile.regional_data.get('spouse_relocation_laboral', False)) else 1
                ...
                if val > 0:
                    deductions.append(Deduction("CTB_TRAS_DESP", val.quantize(Decimal('0.01')), "Traslado de residencia por motivos laborales a municipio despoblado"))
        ...
        if studying_count > 0:
            ...
            if val > 0:
                deductions.append(Deduction("CTB_EST_DESP", val.quantize(Decimal('0.01')), "Gastos de traslado por estudios desde municipios despoblados"))
        ...
        if profile.regional_data.get('cantabria_residence_despoblacion_active', False) and profile.age < 40:
            ...
            num_contribuyentes = 2 if (es_conjunta and profile.regional_data.get('spouse_residence_despoblacion_active', False) and profile.regional_data.get('spouse_age', 40) < 40) else 1
            limit_total = Decimal('500') * num_contribuyentes
            val = min(pct_val, limit_total)
            if val > 0:
                deductions.append(Deduction("CTB_RES_DESP", val.quantize(Decimal('0.01')), "Residencia habitual en municipio con riesgo de despoblamiento"))
        ...
        if apport_socio > 0 or don_dev > 0 or don_fom > 0:
            ...
            if val > 0:
                deductions.append(Deduction("CTB_ECO_SOC", val.quantize(Decimal('0.01')), "Inversiones/donaciones a entidades de Economía Social"))
        ...
        if edu_books > 0 or edu_languages > 0:
            ...
            if val > 0:
                deductions.append(Deduction("CTB_EDUC", val.quantize(Decimal('0.01')), "Gastos de educación (libros e idiomas)"))
        ...
        if dom_ss > 0 and is_employer:
            ...
            if (has_kids and both_work and getattr(profile, 'descendants_count', 0) > 0) or is_monoparental or age_75:
                ...
                if val > 0:
                    deductions.append(Deduction("CTB_DOM", val.quantize(Decimal('0.01')), "Ayuda doméstica por empleados del hogar"))
        ...
        offset_extr = profile.regional_data.get('cantabria_new_residents_foreign_investment_offset_prev_eur', Decimal('0'))
        ...
        if inv_extr > 0 or offset_extr > 0:
            ...
            if val > 0:
                deductions.append(Deduction("CTB_EXTR", val.quantize(Decimal('0.01')), "Inversiones de nuevos residentes procedentes del extranjero"))
        ...
        if reloc_expenses > 0:
            ...
            num_contribuyentes = 2 if (es_conjunta and profile.regional_data.get('spouse_new_resident_relocation', False)) else 1
            ...
            val = min(pct_val, limit_total)
            if val > 0:
                deductions.append(Deduction("CTB_DESP_RES", val.quantize(Decimal('0.01')), "Compensación de gastos por desplazamiento de nuevos residentes"))
        ...
        if empty_count > 0:
            ...
            if total_ded > 0:
                deductions.append(Deduction("CTB_VAC", total_ded.quantize(Decimal('0.01')), "Arrendamiento de viviendas vacías"))
