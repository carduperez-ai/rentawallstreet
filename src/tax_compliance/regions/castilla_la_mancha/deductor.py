    ...
    def calculate(cls, profile: TaxpayerProfile, big: Decimal, bia: Decimal, cuota_auton: Decimal) -> List[Deduction]:
        ...
        if bi_total <= limite_transversal:
            ...
            if births > 0:
                ...
                val = raw_val / coprop
                deductions.append(
                    Deduction("CLM01", val.quantize(Decimal("0.01")), "Por nacimiento o adopción de hijos")
                )
        ...
        if descendants > 0 and net_edu > 0:
            ...
            if limit_per_child > 0:
                ...
                if val > 0:
                    deductions.append(
                        Deduction("CLM04", val.quantize(Decimal("0.01")), "Gastos en libros de texto e idiomas")
                    )
        ...
        if bi_total <= limite_transversal:
            ...
            if daycare_exp > 0:
                ...
                if kids > 0:
                    ...
                    net_exp = max(
                        Decimal("0"), pct_val - profile.regional_data.get("clm_guarderia_subventions_eur", Decimal("0"))
                    )

                    limit_per_kid = (
                        Decimal("250")
                        if profile.regional_data.get("clm_guarderia_child_turns_3", False)
                        else Decimal("500")
                    )
        ...
        if bi_total <= limite_transversal:
            disab_members = getattr(profile, "children_disabled_over65", 0) + getattr(
                profile, "ascendants_disabled_count", 0
            )
            if disab_members > 0:
                coprop = Decimal("1") if es_conjunta else Decimal("2")
                ...
                deductions.append(
                    Deduction(
                        "CLM07", val.quantize(Decimal("0.01")), "Por discapacidad de ascendientes o descendientes"
                    )
                )
        ...
        if bi_total <= limite_transversal:
            ...
            if asc_75 > 0:
                if not profile.regional_data.get("clm_ascendant_residence_public_over_30_days", False):
                    ...
                    deductions.append(
                        Deduction(
                            "CLM09", val.quantize(Decimal("0.01")), "Por cuidado de ascendientes mayores de 75 años"
                        )
                    )
        ...
        if bi_total <= limite_acog:
            ...
            if acog_minors > 0:
                ...
                val = raw_val / coprop
                deductions.append(
                    Deduction("CLM10", val.quantize(Decimal("0.01")), "Acogimiento familiar no remunerado de menores")
                )
        ...
        if don_coop > 0:
            ...
            if val > 0:
                deductions.append(
                    Deduction("CLM17", val.quantize(Decimal("0.01")), "Donaciones cooperación internacional")
                )
        ...
        if don_idi > 0:
            ...
            if val > 0:
                deductions.append(
                    Deduction("CLM18", val.quantize(Decimal("0.01")), "Donaciones investigación y desarrollo")
                )
        ...
        if interest > 0 and profile.age < 40:
            if es_conjunta:
                cap = (
                    Decimal("150")
                    if bi_total <= Decimal("25000")
                    else Decimal("100")
                    if bi_total <= Decimal("36000")
                    else Decimal("0")
                )
            else:
                cap = (
                    Decimal("150")
                    if bi_total <= Decimal("12500")
                    else Decimal("100")
                    if bi_total <= Decimal("27000")
                    else Decimal("0")
                )
        ...
        if estancia and zone in ("extrema", "intensa", "riesgo", "intermedia") and pop > 0:
            ...
            if pct > 0:
                ...
                if val > 0:
                    deductions.append(
                        Deduction("CLM21", val.quantize(Decimal("0.01")), "Residencia habitual en zonas rurales")
                    )
        ...
        if adq_exp > 0 and zone in ("extrema", "intensa") and pop < 5000:
            ...
            if val > 0:
                deductions.append(
                    Deduction(
                        "CLM22", val.quantize(Decimal("0.01")), "Adquisición/rehabilitación vivienda habitual rural"
                    )
                )
        ...
        if bi_total < limite_traslado and profile.regional_data.get("clm_traslado_laboral_despoblacion", False):
            ...
            if year in (2024, 2025):
                num_reloc = (
                    2
                    if (es_conjunta and profile.regional_data.get("clm_traslado_laboral_spouse_relocated", False))
                    else 1
                )
                ...
                if val > 0:
                    deductions.append(
                        Deduction(
                            "CLM23", val.quantize(Decimal("0.01")), "Traslado de residencia por motivos laborales"
                        )
                    )
        ...
        if dog_vet > 0 and bi_total <= limite_transversal:
            ...
            if val > 0:
                deductions.append(
                    Deduction("CLM27", val.quantize(Decimal("0.01")), "Gastos veterinarios perros de asistencia")
                )
        ...
        if base_limite_renta_alq <= limite_alquiler:
            ...
            if (
                profile.regional_data.get("clm_is_monoparental", False) or getattr(profile, "is_monoparental", False)
            ) and rent_paid > 0:
