...
class AragonDeductor:
    ...
    @classmethod
    def calculate_all(cls, profile: TaxpayerProfile, big: Decimal, bia: Decimal, cuota_auton: Decimal = Decimal('0')) -> dict[str, Decimal]:
        deductions = {}
        ...
        val_ara02 = Decimal('0')
        if births > 0 and (getattr(profile, 'children_disabled_33_65', 0) > 0 or getattr(profile, 'children_disabled_over65', 0) > 0):
        ...
        if bi_net <= limite_dep:
            dependents = profile.regional_data.get('dependents_over75_count', 0) + profile.regional_data.get('dependents_disabled_over65_pct_count', 0)
            if dependents > 0:
        ...
        if getattr(profile, 'is_victim_violence_gender_or_terrorism', False) and getattr(profile, 'is_protected_housing', False):
        ...
        if bi_net <= limite_viv_rural and profile.age < 36:
            if profile.regional_data.get('is_first_vivienda_rural', False) and (profile.regional_data.get('vivienda_rural_population_under3k', False) or es_rural):
        ...
        if descendants > 0 and gasto_neto_libros > 0:
            es_num = getattr(profile, 'is_large_family', False) or getattr(profile, 'large_family_category', 'none') != 'none'
        ...
        if bi_total <= limite_70 and profile.age >= 70 and profile.regional_data.get('has_non_capital_general_income', False):
        ...
        val_ara14 = Decimal('0')
        if births > 0 and profile.regional_data.get('resides_under10k_population', False) and profile.regional_data.get('resided_previous_year_under10k', False):
        ...
        if descendants > 0 and gasto_neto_auton > 0 and (getattr(profile, 'children_disabled_33_65', 0) > 0 or getattr(profile, 'children_disabled_over65', 0) > 0):
