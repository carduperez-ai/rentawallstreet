...
class CanariasDeductor:
    ...
    def calculate(cls, profile: TaxpayerProfile, big: Decimal, bia: Decimal, cuota_auton: Decimal) -> List[Deduction]:
        ...
        if don_eco > 0:
            ...
            if val > 0:
                deductions.append(
                    Deduction("CAN_ECO", val.quantize(Decimal("0.01")), "Donaciones con finalidad ecológica")
                )
            ...
            if val > 0:
                deductions.append(
                    Deduction(
                        ...
                        "Donaciones rehabilitación/conservación patrimonio histórico",
                    )
                )
            ...
            if val > 0:
                deductions.append(
                    Deduction(
                        "CAN_CULT_DEP_RDI",
                        val.quantize(Decimal("0.01")),
                        "Donaciones fines culturales, deportivos o investigación",
                    )
                )
        ...
        if don_esl > 0:
            ...
            if val > 0:
                deductions.append(
                    Deduction("CAN_ESL", val.quantize(Decimal("0.01")), "Donaciones a entidades sin ánimo de lucro")
                )
        ...
        if bi_total <= limite_general:
            ...
            if count_out > 0 or count_same > 0:
                ...
                if val > 0:
                    deductions.append(
                        Deduction("CAN_EST_SUP", val.quantize(Decimal("0.01")), "Gastos estudios educación superior")
                    )
        ...
        if bi_total <= limite_general:
            ...
            if exp_nosup > 0:
                ...
                if desc_count > 0:
                    ...
                    if val > 0:
                        deductions.append(
                            Deduction("CAN_EST_NOSUP", val.quantize(Decimal("0.01")), "Gastos estudios no superiores")
                        )

        # 8. CAN_TRASLADO: Por trasladar la residencia habitual a otra isla para realizar actividad económica
        if profile.regional_data.get("canarias_relocation_island_active", False) and profile.regional_data.get(
            "canarias_relocation_island_year", 2025
        ) in (2024, 2025):
            if bi_total <= limite_general:
                ...
                if val > 0:
                    deductions.append(
                        Deduction(
                            "CAN_TRASLADO", val.quantize(Decimal("0.01")), "Traslado de residencia habitual a otra isla"
                        )
                    )
        ...
        val_univ = profile.regional_data.get("canarias_new_entities_investment_univ_rdi_eur", Decimal("0")) * Decimal(
            "0.30"
        )
        val_univ = min(val_univ, Decimal("6000"))

        val_coop = profile.regional_data.get("canarias_new_entities_investment_labor_coop_eur", Decimal("0")) * Decimal(
            "0.30"
        )
        ...
        if val_acc > 0:
            deductions.append(
                Deduction(
                    "CAN_ACCIONES", val_acc.quantize(Decimal("0.01")), "Inversión en acciones de nuevas entidades"
                )
            )
        ...
        if bi_total <= limite_general:
            ...
            if births > 0:
                ...
                if val > 0:
                    deductions.append(
                        Deduction("CAN_NACIMIENTO", val.quantize(Decimal("0.01")), "Nacimiento o adopción de hijos")
                    )
        ...
        if bi_total <= limite_general:
            ...
            if val_disc_edad > 0:
                deductions.append(
                    Deduction(
                        "CAN_DISCAP_EDAD",
                        val_disc_edad.quantize(Decimal("0.01")),
                        "Discapacidad o edad del contribuyente",
                    )
                )
        ...
        if getattr(profile, "is_large_family", False):
            is_special = (
                getattr(profile, "is_large_family_special", False)
                or getattr(profile, "large_family_category", "") == "special"
            )
            has_disab = (
                profile.regional_data.get("balears_is_disabled_member", False)
        ...
        if bi_total <= limite_general:
            ...
            if rent > 0 and getattr(profile, "is_rent_habitual", False):
                if rent > (bi_total * Decimal("0.10")):
                    ...
                    if val > 0:
                        deductions.append(
                            Deduction("CAN_ALQUILER", val.quantize(Decimal("0.01")), "Alquiler vivienda habitual")
                        )
        ...
        if bi_total <= limite_general:
            ...
            if rent > 0:
                ...
                if val > 0:
                    deductions.append(
                        Deduction("CAN_ALQ_DACION", val.quantize(Decimal("0.01")), "Arrendamiento por dación en pago")
                    )
        ...
        if val_arrend_adeq > 0:
            deductions.append(
                Deduction(
                    "CAN_ARREND_ADEQ",
                    val_arrend_adeq.quantize(Decimal("0.01")),
                    "Adecuación inmueble para arrendamiento",
                )
            )
        elif val_arrend_insur > 0:
            deductions.append(
                Deduction("CAN_ARREND_INSUR", val_arrend_insur.quantize(Decimal("0.01")), "Seguro impago de alquiler")
            )
        ...
        if prop_count > 0:
            ...
            if val > 0:
                deductions.append(
                    Deduction(
                        "CAN_ARREND_MERCADO", val.quantize(Decimal("0.01")), "Puesta de vivienda en mercado de alquiler"
                    )
                )
        ...
        if health_exp > 0 or optical_exp > 0:
            ...
            if val > 0:
                deductions.append(
                    Deduction("CAN_MED_EXP", val.quantize(Decimal("0.01")), "Gastos de enfermedad y aparatos médicos")
                )
        ...
        if dep_count > 0 and bi_total <= limite_general:
            ...
            if val > 0:
                deductions.append(
                    Deduction(
                        "CAN_DEP_DISCAP", val.quantize(Decimal("0.01")), "Familiares dependientes con discapacidad"
                    )
                )
        ...
        if ss_cont > 0:
            ...
            if elig_child or elig_age_75 or elig_age_65_disc:
                ...
                if val > 0:
                    deductions.append(
                        Deduction("CAN_EMP_HOGAR", val.quantize(Decimal("0.01")), "Contratación de empleados de hogar")
                    )
