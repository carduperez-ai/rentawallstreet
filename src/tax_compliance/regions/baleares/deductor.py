...
class BalearsDeductor:
    ...
    def calculate(cls, profile: TaxpayerProfile, big: Decimal, bia: Decimal, cuota_auton: Decimal) -> List[Deduction]:
        ...
        es_monoparental_2_kids = es_monoparental and (conciliacion_kids >= 2 or getattr(profile, 'descendants_count', 0) >= 2)
        ...
        es_discapacitado_33 = profile.disability_grade >= 33 or profile.regional_data.get('balears_is_disabled_member', False)
        es_discapacitado_psiquica_33 = profile.regional_data.get('balears_is_disabled_member_psiquica', False)
        es_autonomo_183_dias = profile.regional_data.get('balears_autoocupacion_mantenimiento_un_ano', False) or profile.regional_data.get('is_autonomo', False)

        tiene_derecho_minimo_discapacidad = (
            profile.disability_grade >= 33 or
            getattr(profile, 'children_disabled_over65', 0) > 0 or
            getattr(profile, 'ascendants_disabled_count', 0) > 0 or
            profile.regional_data.get('descendants_disabled_count', 0) > 0
        )

        es_colectivo_b = (
            es_menor_30 or
            es_discapacitado_33 or
            tiene_derecho_minimo_discapacidad or
            es_numerosa or
            es_monoparental_2_kids or
            es_autonomo_183_dias
        )
        ...
        if subv_catastrofe > 0:
            ...
            deductions.append(Deduction("BAL03", (subv_catastrofe * tipo_medio).quantize(Decimal('0.01')), "Subvenciones por Zona Afectada por Emergencia"))
        ...
        if bi_total <= limite_bi_cruzado:
            ...
            if libros_gastos > 0:
                ...
                if hijos > 0:
                    ...
                    coprop = Decimal('2') if (not es_conjunta and getattr(profile, 'custody_shared', False)) else Decimal('1')
        ...
        if bi_total <= limite_arrendador:
            ...
            if otros_gastos_arr > 0 and profile.regional_data.get('balears_is_landlord_first_year', False):
                limite_max = Decimal('1800') if profile.regional_data.get('balears_landlord_is_ibavi', False) else Decimal('1500')
        ...
        if val_acciones > 0:
            ...
            deductions.append(Deduction("BAL22", min(val_acciones, limite_global), "Inversión en Acciones de Nuevas Entidades"))
