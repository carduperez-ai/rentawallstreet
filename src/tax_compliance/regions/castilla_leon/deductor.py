...
class CastillaLeonDeductor:
    ...
    def calculate(cls, profile: TaxpayerProfile, big: Decimal, bia: Decimal, cuota_auton: Decimal) -> List[Deduction]:
        ...
        if births > 0:
            ...
            if profile.regional_data.get('cyl_birth_is_disabled', False) or profile.regional_data.get('cyl_births_disabled_count', 0) > 0:
                val_hijo *= Decimal('2')
                
            val_total = val_hijo * births
            val_prorated = val_total / coprop
            deductions.append(Deduction("CYL02", val_prorated.quantize(Decimal('0.01')), "Por nacimiento o adopción de hijos"))
        ...
        if multiple_count >= 2:
            ...
            deductions.append(Deduction("CYL_MUL", val_prorated.quantize(Decimal('0.01')), "Por partos o adopciones múltiples"))
        elif profile.regional_data.get('cyl_multiple_births_past_years_count', 0) > 0:
            past_multiple = profile.regional_data.get('cyl_multiple_births_past_years_count', 0)
            val_prorated = (Decimal('901') * past_multiple) / coprop
            deductions.append(Deduction("CYL_MUL", val_prorated.quantize(Decimal('0.01')), "Por partos múltiples en años anteriores"))
        ...
        if bi_net <= limite_renta:
            ...
            if (daycare > 0 or empleada > 0) and desc_count > 0:
                ...
                if val_prorated > 0:
                    deductions.append(Deduction("CYL_CUID", val_prorated.quantize(Decimal('0.01')), "Por cuidado de hijos menores"))
            ...
            if hogar_ss > 0 and desc_count > 0:
                ...
                if val_prorated > 0:
                    deductions.append(Deduction("CYL_HOG", val_prorated.quantize(Decimal('0.01')), "Por cuotas a la SS de empleados de hogar"))
            ...
            if discap_grade >= 33 and not reside_publico:
                ...
                if val_disc > 0:
                    ...
                    deductions.append(Deduction("CYL_DISC", val_disc.quantize(Decimal('0.01')), "Por discapacidad del contribuyente"))
            ...
            if v_rural_exp > 0 and age < 36:
                ...
                if es_primera and valor_casa < Decimal('150000'):
                    ...
                    if val_prorated > 0:
                        deductions.append(Deduction("CYL_VIV_RURAL", val_prorated.quantize(Decimal('0.01')), "Adquisición/Rehabilitación de vivienda por jóvenes en medio rural"))

            # 9. CYL01 — Arrendamiento de vivienda habitual por jóvenes
            # 20% (cap 459€) o 25% (cap 612€ rural)
            if (getattr(profile, 'is_rent_habitual', False)
                ...
                if val_prorated > 0:
                    deductions.append(Deduction("CYL01", val_prorated.quantize(Decimal('0.01')), "Por arrendamiento de vivienda habitual por jóvenes"))
