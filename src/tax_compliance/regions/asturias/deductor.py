...
class AsturiasDeductor:
    ...
    def calculate(cls, profile: TaxpayerProfile, big: Decimal, bia: Decimal, cuota_auton: Decimal) -> List[Deduction]:
        ...
        if bi_total <= limite_comun:
            if (
                ...
                es_colectivo_especial = (
                    profile.age < 35
                    or getattr(profile, "is_large_family", False)
                    or getattr(profile, "is_monoparental", False)
                    or getattr(profile, "is_victim_violence_gender_or_terrorism", False)
                    or profile.regional_data.get("resides_in_asturias_despoblacion", False)
                )
        ...
        if getattr(profile, "descendants_count", 0) > 0 and getattr(profile, "custody_shared", False):
            deductions.append(
                Deduction("AST10", Decimal("500") * profile.descendants_count, "Acogimiento familiar de menores")
            )
        ...
        if traslado > 0:
            lim_max = (
                Decimal("2000")
                if profile.regional_data.get("asturias_is_relocation_highly_qualified", False)
                else Decimal("1000")
            )
        ...
        if gastos_arrendador > 0:
            ...
            deductions.append(
                Deduction("AST24", min(gastos_arrendador, limite_arr), "Gastos del arrendador por vivienda sostenible")
            )
        ...
        if desc_acc > 0:
            deductions.append(
                Deduction("AST26", Decimal("1000") * desc_acc, "Descendientes por fallecimiento de progenitor laboral")
            )
        ...
        if inv_acciones > 0:
            deductions.append(
                Deduction(
                    "AST27",
                    min(inv_acciones * Decimal("0.30"), Decimal("6000")),
                    "Inversión en acciones de nuevas entidades",
                )
            )
