...
def _to_int(valor, default=0) -> int:
    ...
    try: return int(_to_dec(valor).to_integral_value())
...
def _build_profile(form) -> 'TaxpayerProfile':
    ...
    return TaxpayerProfile(
        ...
        large_family_category='general' if family_type == 'large_general' else ('special' if family_type == 'large_special' else 'none'),
...
from src.domain.fiscal_entities import WorkIncome, RentalIncome, OtherIncome
from src.ingestion.file_classifier import FileClassifier

# Importación de Especialistas
from mainstock import StockSpecialist
...
if os.path.exists(secret_file):
    with open(secret_file, 'r') as f: default_secret = f.read().strip()
else:
    default_secret = os.urandom(24).hex()
    try:
        with open(secret_file, 'w') as f: f.write(default_secret)
    except: pass
    ...
    try:
        ...
        for folder in os.listdir(UPLOAD_FOLDER):
            ...
            if os.path.isdir(fpath) and os.path.getmtime(fpath) < time.time() - 43200:
                shutil.rmtree(fpath)
        print("[STARTUP] Garbage Collector finalizado.")
    except Exception as e:
        print(f"(!) Error en purga inicial: {e}")
print("[STARTUP] Clasificador activo y carpeta segura.")
...
def index():
    ...
    if not inventory:
        ...
        print("  [DISK SYNC] Estado: CARPETA VACÍA. Sesión reiniciada.")
...
def calculate():
    try:
        ...
        if request.form.get('mortgage_pre2013') == 'on' and not request.form.get('mortgage_acquisition_date', '').strip():
        ...
        if not physical_files:
            ...
            classified_files = {}
            print("LOG: Carpeta vacía detectada. Sesión sincronizada a CERO.")
        else:
        ...
        ALL_ADAPTERS = [
            IBKRAdapter(),
            EToroAdapter(),
            KrakenAdapter(), KuCoinAdapter(), CoinbaseAdapter(),
            UniversalAdapter()
        ]
        ...
        for f, meta in classified.items():
            ...
            for adapter in ALL_ADAPTERS:
                if adapter.can_handle(meta):
                    ...
                    if not t and not d:
                        flash(f"Error de Ingesta: El adaptador '{adapter.platform_id}' reconoció el archivo '{f}' pero no pudo extraer transacciones.", "danger")
        ...
        # --- ESPECIALISTA BOLSA (mainstock.py) ---
        stock_specialist = StockSpecialist(tax_year=fiscal_year)
        # Normalizar nombres de plataforma para el especialista de bolsa
        for f, meta in classified.items():
            if meta.platform.upper() in ('T212', 'TRADING212', 'TRADING 212'):
                meta.platform = 'TRADING212'
            if meta.platform.upper() in ('DEGIRO', 'DE GIRO'):
                meta.platform = 'DEGIRO'
                
        stock_specialist.process_files(user_folder, classified, all_stock_trades_orphan, all_stock_divs_orphan)

        # --- ORQUESTADOR GLOBAL (Simulador Histórico V5) ---
        from src.accounting.interwallet_controller import InterwalletController
        
        stock_raw_trades = stock_specialist.get_all_trades()
        controller = InterwalletController(
            raw_crypto_trades=all_crypto_trades,
            raw_stock_trades=stock_raw_trades
        )
        ...
        stock_specialist.engine.warnings = controller.stock_warnings
        stock_divs = stock_specialist.engine.dividends
        ...
        # 5. Publicación
        all_warnings = crypto_engine.warnings + stock_specialist.get_warnings()
        
        return render_template('result.html', summary=summary_conj, summary_ind1=summary_ind1, summary_ind2=summary_ind2, warnings=all_warnings)
...
def download_audit(session_hash):
    ...
    is_valid = re.match(r'^[a-fA-F0-9]{32,64}$', session_hash) or session_hash in ('session-owner-hash', 'session-completed-hash')
    if not is_valid:
