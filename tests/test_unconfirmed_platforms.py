...
def procesar_impuesto(trades, dividends):
    ...
    dto = calc.calculate()
    print(
        f"GPP Neto: {dto.raw_summary['gpp_neto']} | Base Ahorro: {dto.savings_taxable_base} | Cuota: {dto.final_result}"
    )
...
def test_coinbase_parser(tmp_path):
    ...
    pd.DataFrame(
        {
            "timestamp": ["2025-01-10T10:00:00Z", "2025-02-10T10:00:00Z"],
            "transaction type": ["Buy", "Sell"],
            "asset": ["BTC", "BTC"],
            "quantity transacted": [1.0, 1.0],
            "spot price currency": ["EUR", "EUR"],
            "spot price at transaction": [10000.0, 20000.0],
            "subtotal": [10000.0, 20000.0],
            "fees and/or spread": [10.0, 10.0],
            "total (inclusive of fees and/or spread)": [10010.0, 19990.0],
            "notes": ["", ""],
        }
...
def test_etoro_parser(tmp_path):
    ...
    with pd.ExcelWriter(mock_file) as writer:
        pd.DataFrame(
            {
                "Date": ["10/01/2025 10:00:00", "20/01/2025 10:00:00"],
                "Type": ["Position closed", "Position closed"],
                "Amount": [10000.0, 20000.0],
                "Units": [1.0, -1.0],
                "Action": ["Buy Bitcoin", "Sell Bitcoin"],
                "Asset": ["BTC", "BTC"],
                "Spread": [10.0, 10.0],
                "Profit(USD)": [0.0, 10000.0],
                "ISIN": ["", ""],
            }
        ).to_excel(writer, sheet_name="Closed Positions", index=False)
        pd.DataFrame(
            {
                "Date": ["10/01/2025 10:00:00", "20/01/2025 10:00:00"],
                ...
                "Details": ["", ""],
                "Amount": [10000.0, -20000.0],
                "Realized Equity Change": [0.0, 10000.0],
                "Realized Equity": [10000.0, 20000.0],
                "Balance": [10000.0, 20000.0],
                "Position ID": ["1", "1"],
            }
        ).to_excel(writer, sheet_name="Account Activity", index=False)
        pd.DataFrame(
            {
                "Date of Payment": ["15/02/2025 10:00:00"],
                "Instrument Name": ["AAPL"],
                "Net Dividend Received (USD)": [5.0],
                "Withholding Tax Rate (%)": [15.0],
            }
        ).to_excel(writer, sheet_name="Dividends", index=False)
    ...
    pd.DataFrame(
        {
            ...
            "Symbol": ["AAPL", "AAPL"],
            "Quantity": [100, -100],
            "TradePrice": [150.0, 250.0],
            "Proceeds": [-15000.0, 25000.0],
            "IBCommission": [-1.0, -1.0],
            "TradeDate": ["2025-01-10", "2025-02-10"],
            "Currency": ["EUR", "EUR"],
        }
    ).to_csv(mock_file, index=False)
    ...
    pd.DataFrame(
        {
            "txid": ["t1", "t2"],
            "refid": ["r1", "r2"],
            "time": ["2025-01-10 10:00:00", "2025-02-10 10:00:00"],
            "type": ["trade", "trade"],
            "subtype": ["", ""],
...
def test_revolut_parser(tmp_path):
    ...
    pd.DataFrame(
        {
            "Type": ["BUY", "SELL"],
            "Date": ["2025-01-10T10:00:00Z", "2025-02-10T10:00:00Z"],
            "Ticker": ["AAPL", "AAPL"],
            "Quantity": [100, 100],
            "Price": [150.0, 250.0],
            "Total Amount": [15000.0, 25000.0],
            "Fee": [1.0, 1.0],
            "Currency": ["USD", "USD"],
        }
    ).to_csv(mock_file, index=False)
...
def test_interwallet_unconfirmed_platforms(tmp_path):
    ...
    pd.DataFrame(
        {
            ...
            "spot price at transaction": [10000.0],
            "subtotal": [10000.0],
            "fees and/or spread": [10.0],
            "total (inclusive of fees and/or spread)": [10010.0],
            "notes": [""],
        }
        ...
        pd.DataFrame(
            {
                ...
                "Amount": [1000.0],
                "Units": [10.0],
                "Open Date": ["10/01/2025 10:00:00"],
                "Close Date": ["20/01/2025 10:00:00"],
                "Spread": [1.0],
                "Profit(USD)": [100.0],
            }
        ).to_excel(writer, sheet_name="Closed Positions", index=False)
    ...
    ).to_csv(rvf, index=False)
    t, _ = revolut_reader.parse(str(rvf))
    clasificar(t)

    all_stock.sort(key=lambda x: x.date)
    all_crypto.sort(key=lambda x: x.date)
