...
def test_cantabria_alquiler_general_and_despoblacion():
    ...
    res = {
        d.box_id: d.value for d in CantabriaDeductor.calculate(profile, Decimal("20000"), Decimal("0"), Decimal("2000"))
    }
    ...
    profile_general = ProfileBuilder.from_dict(
        dict(
            region="cantabria",
            age=30,
    ...
    res_general = {
        d.box_id: d.value
        for d in CantabriaDeductor.calculate(profile_general, Decimal("20000"), Decimal("0"), Decimal("2000"))
    }
...
def test_cantabria_nacimiento_and_cuidado():
    ...
    profile_discap = ProfileBuilder.from_dict(
        dict(
            region="cantabria",
            age=30,
            minimo_personal_familiar=Decimal("5550"),
    ...
    res_discap = {
        d.box_id: d.value
        for d in CantabriaDeductor.calculate(profile_discap, Decimal("20000"), Decimal("0"), Decimal("2000"))
    }
...
def test_cantabria_obras_mejora():
    ...
    res = {
        d.box_id: d.value for d in CantabriaDeductor.calculate(profile, Decimal("30000"), Decimal("0"), Decimal("3000"))
    }
    ...
    res_limit = {
        d.box_id: d.value
        for d in CantabriaDeductor.calculate(profile_limit, Decimal("30000"), Decimal("0"), Decimal("3000"))
    }
...
def test_cantabria_donaciones_limite_conjunto():
    ...
    res = {
        d.box_id: d.value for d in CantabriaDeductor.calculate(profile, Decimal("2000"), Decimal("0"), Decimal("500"))
    }
...
def test_cantabria_gastos_enfermedad():
    ...
    res = {
        d.box_id: d.value for d in CantabriaDeductor.calculate(profile, Decimal("20000"), Decimal("0"), Decimal("1000"))
    }
    assert res.get("CTB_ENF") == Decimal("200.00")

    # Excediendo límite de base imponible -> excluido
    res_excl = {
        d.box_id: d.value for d in CantabriaDeductor.calculate(profile, Decimal("35000"), Decimal("0"), Decimal("1000"))
    }
...
def test_cantabria_guarderias_and_monoparental():
    ...
    res = {
        d.box_id: d.value for d in CantabriaDeductor.calculate(profile, Decimal("15000"), Decimal("0"), Decimal("1000"))
    }
