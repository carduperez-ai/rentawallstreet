...
def test_bal01_alquiler_general():
    ...
    profile = ProfileBuilder.from_dict(dict(region='baleares', 
        age=34,
        is_rent_habitual=True,
        rent_paid_annual_eur=Decimal('4000'),
        joint_declaration=False
    ))
    res = {d.box_id: d.value for d in BalearsDeductor.calculate(profile, Decimal('20000'), Decimal('0'), Decimal('1000'))}
    assert res.get("BAL01") == Decimal('530')

    # Excede límite BI (34.000€ > 33k) -> Excluido
    res_high = {d.box_id: d.value for d in BalearsDeductor.calculate(profile, Decimal('34000'), Decimal('0'), Decimal('1000'))}
...
def test_bal01_alquiler_colectivo_b():
    ...
    res = {d.box_id: d.value for d in BalearsDeductor.calculate(profile, Decimal('25000'), Decimal('0'), Decimal('1000'))}
...
def test_bal04_books_and_material():
    ...
    profile_b = ProfileBuilder.from_dict(dict(region='baleares', 
        age=40,
        descendants_count=1,
    ...
    res_b = {d.box_id: d.value for d in BalearsDeductor.calculate(profile_b, Decimal('20000'), Decimal('0'), Decimal('1000'))}
...
def test_bal17_y_bal20_incompatibilidad():
    ...
    profile_care_wins = ProfileBuilder.from_dict(dict(region='baleares', 
        age=40,
        balears_conciliacion_expenses_eur=Decimal('1200'),
        balears_conciliacion_kids_count=1,
    ...
    res_care = {d.box_id: d.value for d in BalearsDeductor.calculate(profile_care_wins, Decimal('20000'), Decimal('0'), Decimal('1000'))}
...
def test_bal18_birth_reduction_50():
    ...
    profile = ProfileBuilder.from_dict(dict(region='baleares', 
        age=35,
        balears_births_count=1,
        balears_birth_order=1,
        joint_declaration=False
    ))
    res = {d.box_id: d.value for d in BalearsDeductor.calculate(profile, Decimal('40000'), Decimal('0'), Decimal('1000'))}
    ...
    res_high = {d.box_id: d.value for d in BalearsDeductor.calculate(profile, Decimal('60000'), Decimal('0'), Decimal('1000'))}
