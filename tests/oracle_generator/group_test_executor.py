...
def print_audit_report(report):
    ...
    print(f"  Result: {'[OK] PASSED' if report.all_passed else '[!!] FAILED'} ({report.passed_count} ok, {report.failed_count} errors)")
    
    for finding in report.findings:
        symbol = "[V]" if finding.passed else "[X]"
        desc = finding.description.replace('€', 'EUR').replace('≥', '>=').replace('≤', '<=').replace('×', 'x').replace('−', '-')
