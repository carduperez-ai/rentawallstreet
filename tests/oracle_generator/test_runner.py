...
def _build_profile(inp: dict) -> Optional[TaxpayerProfile]:
    ...
    large_cat = (
        "general" if family_type == "large_general" else ("special" if family_type == "large_special" else "none")
    )
    return ProfileBuilder.from_dict(
        dict(
    ...
    for i, d in enumerate(inp.get("dividends", [])):
        result.append(
            Dividend(
                ...
                withholding_spain_eur=_d(
                    d.get("withholding", d.get("withholding_spain", d.get("withholding_spain_eur", 0)))
                ),
    ...
    if cases is None:
        ...
        if os.path.exists(_JSON_PATH):
            with open(_JSON_PATH, encoding="utf-8") as f:
                for c in json.loads(f.read(), parse_float=Decimal):
                    if c["name"] not in seen:
                        all_cases.append(c)
                        seen.add(c["name"])
        # Generadores añaden casos sintéticos no presentes en el JSON
        for c in generate_salary_thresholds() + generate_family_cases() + generate_deduction_cases():
            if c["name"] not in seen:
                all_cases.append(c)
                seen.add(c["name"])
        cases = all_cases
...
def _check_art20(inp: dict, result: dict, findings: list) -> None:
    ...
    if is_married_joint:
        ...
        tramo = (
            "1"
            if rnt_art20_s1 <= Decimal("14852")
            else ("2" if rnt_art20_s1 <= Decimal("17673.52") else ("3" if rnt_art20_s1 <= Decimal("19747.50") else "4"))
        )
    ...
    if otras_rentas > Decimal("6500"):
        _append(
            findings,
            "ART20_OTRAS_RENTAS_GATE",
            "Art. 20 LIRPF",
            f"otras_rentas={otras_rentas:.2f}€ > 6500€: reducción Art.20 debe ser 0",
            _ZERO,
            rnt_trabajo - work_neto if work_neto > _ZERO else None,
            True,
            "INFO",
        )
...
def _check_art51_52(inp: dict, result: dict, findings: list) -> None:
    ...
    if is_joint:
        ...
        rnt_s1 = max(
            _d(result.get("rendimiento_neto_trabajo", _ZERO))
            - (_d(inp.get("spouse_income", 0)) - _d(inp.get("spouse_ss", 0)) - Decimal("2000")),
            _ZERO,
        )
...
def _check_art81_81bis(inp: dict, result: dict, findings: list) -> None:
    ...
    if family_type == "large_general":
        ...
        _append(
            findings,
            "ART81BIS_FN_ESPECIAL",
            "Art. 81bis LIRPF",
            "Familia numerosa especial: 2400€",
            Decimal("2400"),
            actual,
            passed,
        )
...
def _check_art84(inp: dict, result: dict, findings: list) -> None:
    ...
    if joint and married and not monoparental:
        ...
        _append(
            findings,
            "ART84_CONJUNTA_CASADO",
            "Art. 84 LIRPF",
            "Declaración conjunta matrimonio: 3400€",
            Decimal("3400"),
            actual,
            passed,
        )
        ...
        _append(
            ...
            "ART84_CONJUNTA_MONO",
            "Art. 84 LIRPF",
            "Declaración conjunta monoparental: 2150€",
            Decimal("2150"),
            actual,
            passed,
        )
...
def _check_systemic(inp: dict, result: dict, findings: list) -> None:
    ...
    _append(
        findings,
        "SYS_CUOTA_LIQUIDA_NN",
        "Invariante sistémico",
    ...
    for result_key, exp_key, rule_id, label in checks:
        ...
        if exp_val == 0.0:
            _append(
                ...
                rule_id,
                "Oracle Test",
                f"{label}: sin valor oracle (expected=0.0, se omite)",
                None,
                None,
                True,
                "INFO",
            ...
            _append(findings, rule_id, "Oracle Test", f"{label}: oracle={exp_val}€", exp_d, actual, passed)
