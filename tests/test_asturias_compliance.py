...
def test_ast01_alquiler_general():
    ...
    profile = ProfileBuilder.from_dict(dict(region='asturias', 
        age=40,
        is_rent_habitual=True,
        rent_paid_annual_eur=Decimal('6000'),
        joint_declaration=False
    ))
    res = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile, Decimal('20000'), Decimal('0'), Decimal('1000'))}
    assert res.get("AST01") == Decimal('500')

    # Caso 2: Supera base imponible individual (36.000€ > 35k) -> Excluido
    res_high = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile, Decimal('36000'), Decimal('0'), Decimal('1000'))}
...
def test_ast01_alquiler_colectivo_especial():
    ...
    res = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile, Decimal('25000'), Decimal('0'), Decimal('1000'))}
    assert res.get("AST01") == Decimal('1200')

    # Caso 2: Joven (< 35 años), individual, alquiler 6.000€ -> 30% = 1.800€, topado a 1.500€
    res_capped = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile, Decimal('25000'), Decimal('0'), Decimal('1000'))}
    assert res_capped.get("AST01") == Decimal('1200')
    
    profile_high_rent = ProfileBuilder.from_dict(dict(region='asturias', 
        age=30,
        is_rent_habitual=True,
        rent_paid_annual_eur=Decimal('6000'),
        joint_declaration=False
    ))
    res_capped2 = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile_high_rent, Decimal('25000'), Decimal('0'), Decimal('1000'))}
...
def test_ast03_acogimiento_mayores_65():
    ...
    res_high = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile, Decimal('27000'), Decimal('0'), Decimal('1000'))}
...
def test_ast12_centros_cero_tres():
    ...
    profile_despoblacion = ProfileBuilder.from_dict(dict(region='asturias', 
        age=35,
        children_under_3=1,
    ...
    res = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile_despoblacion, Decimal('20000'), Decimal('0'), Decimal('1000'))}
    assert res.get("AST12") == Decimal('900')
    assert "AST20" not in res

    # Caso 2: Con ayudas públicas de 200€ -> Deducción neta = 900 - 200 = 700€
    profile_with_grants = ProfileBuilder.from_dict(dict(region='asturias', 
        age=35,
        children_under_3=1,
        daycare_expenses_eur=Decimal('3000'),
    ...
    res_grants = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile_with_grants, Decimal('20000'), Decimal('0'), Decimal('1000'))}
...
def test_ast13_libros_y_material():
    ...
    res = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile, Decimal('5000'), Decimal('0'), Decimal('1000'))}
    assert res.get("AST13") == Decimal('50')

    # Caso 2: Individual, familia numerosa, BI=20.000€ -> Límite 75€, gasto 100€ -> 75€
    profile_large = ProfileBuilder.from_dict(dict(region='asturias', 
        age=40,
        descendants_count=3,
    ...
    res_large = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile_large, Decimal('20000'), Decimal('0'), Decimal('1000'))}
...
def test_ast25_gastos_vitales_jovenes():
    ...
    profile_24 = ProfileBuilder.from_dict(dict(region='asturias', 
        age=24,
        joint_declaration=False
    ))
    res = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile_24, Decimal('25000'), Decimal('0'), Decimal('1000'))}
    assert res.get("AST25") == Decimal('2000')

    # Caso 2: Joven de 28 años, BI=25.000€ -> Deducción = 1.500€
    profile_28 = ProfileBuilder.from_dict(dict(region='asturias', 
        age=28,
        joint_declaration=False
    ))
    res_28 = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile_28, Decimal('25000'), Decimal('0'), Decimal('1000'))}
    assert res_28.get("AST25") == Decimal('1500')

    # Caso 3: Joven de 32 años, BI=25.000€ -> Deducción = 1.000€
    profile_32 = ProfileBuilder.from_dict(dict(region='asturias', 
        age=32,
        joint_declaration=False
    ))
    res_32 = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile_32, Decimal('25000'), Decimal('0'), Decimal('1000'))}
    assert res_32.get("AST25") == Decimal('1000')

    # Caso 4: Supera base imponible (> 28k) -> Excluido
    res_high = {d.box_id: d.value for d in AsturiasDeductor.calculate(profile_24, Decimal('29000'), Decimal('0'), Decimal('1000'))}
