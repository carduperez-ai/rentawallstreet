...
def test_other_income_titularidad_bounds():
    # Should work
    income = OtherIncome(
        date=datetime.now(), description="Lotería", gain_loss_eur=Decimal('100000'), category="premios",
        subtype=IncomeSubtype.LOTERIAS_OFICIALES, porcentaje_titularidad=Decimal('50.00')
    )
    ...
    with pytest.raises(ValueError, match="Decimal estricto"):
        OtherIncome(
            date=datetime.now(), description="Lotería", gain_loss_eur=100000.0, category="premios"
        )
...
def test_family_member_limits():
    ...
    m2 = FamilyMember(role=FamilyRole.DESCENDANT, age=20, rentas_obtenidas=Decimal('2000'), presenta_declaracion_independiente=True)
