...
def test_ara01_nacimiento_tercer_hijo():
    ...
    profile = ProfileBuilder.from_dict(
        dict(region="aragon", age=35, descendants_count=3, birth_count_current_year=1, joint_declaration=False)
    )
    ...
    profile_rural = ProfileBuilder.from_dict(
        dict(
            region="aragon",
            age=35,
            descendants_count=3,
...
def test_ara02_nacimiento_discapacidad():
    ...
    profile = ProfileBuilder.from_dict(
        dict(region="aragon", age=35, birth_count_current_year=1, children_disabled_33_65=1)
    )
...
def test_ara03_adopcion_internacional():
    profile = ProfileBuilder.from_dict(dict(region="aragon", age=35, ara_international_adoption_count=1))
    _raw = AragonDeductor.calculate_all(profile, Decimal("20000"), Decimal("0"), Decimal("1000"))
    res = _raw
    assert res.get("ARA03") == Decimal("600")

    profile_rural = ProfileBuilder.from_dict(
        dict(region="aragon", age=35, ara_international_adoption_count=1, ara_resides_in_rural_despoblacion=True)
    )
    res_rural = AragonDeductor.calculate_all(profile_rural, Decimal("20000"), Decimal("0"), Decimal("1000"))
...
def test_ara04_cuidado_dependientes():
    ...
    profile = ProfileBuilder.from_dict(dict(region="aragon", age=35, ara_dependents_over75_count=1))
    _raw = AragonDeductor.calculate_all(profile, Decimal("15000"), Decimal("0"), Decimal("1000"))
    res = _raw
    ...
    # Rural -> 300€
    profile_rural = ProfileBuilder.from_dict(
        dict(region="aragon", age=35, ara_dependents_over75_count=1, ara_resides_in_rural_despoblacion=True)
    )
    res_rural = AragonDeductor.calculate_all(profile_rural, Decimal("15000"), Decimal("0"), Decimal("1000"))
...
def test_ara05_donaciones_ecologicas():
    profile = ProfileBuilder.from_dict(
        dict(region="aragon", age=35, ara_donations_ecological_research_eur=Decimal("500"))
    )
...
def test_ara07_inversion_mab():
    profile = ProfileBuilder.from_dict(dict(region="aragon", age=35, ara_mab_investment_eur=Decimal("10000")))
    # 20% de 10.000€ = 2.000€
    _raw = AragonDeductor.calculate_all(profile, Decimal("20000"), Decimal("0"), Decimal("1000"))
    res = _raw
...
def test_ara08_inversion_entidades_nuevas():
    profile = ProfileBuilder.from_dict(dict(region="aragon", age=35, ara_new_entities_investment_eur=Decimal("110000")))
...
def test_ara10_libros_texto():
    ...
    profile = ProfileBuilder.from_dict(
        dict(region="aragon", age=40, descendants_count=1, ara_school_material_expenses_eur=Decimal("80"))
    )
    ...
    profile_grant = ProfileBuilder.from_dict(
        dict(
            region="aragon",
            age=40,
            descendants_count=1,
...
def test_ara11_arrendamiento_dacion_pago():
    profile = ProfileBuilder.from_dict(dict(region="aragon", age=35, ara_dacion_en_pago_rent_paid_eur=Decimal("3000")))
    # 10% de 3000€ = 300€
...
def test_ara12_arrendamiento_social():
    profile = ProfileBuilder.from_dict(dict(region="aragon", age=45, ara_social_rental_net_income_eur=Decimal("3000")))
...
def test_ara13_mayores_70():
    profile = ProfileBuilder.from_dict(dict(region="aragon", age=72, ara_has_non_capital_general_income=True))
    _raw = AragonDeductor.calculate_all(profile, Decimal("20000"), Decimal("0"), Decimal("1000"))
    res = _raw
...
def test_ara15_guarderia():
    profile = ProfileBuilder.from_dict(
        dict(region="aragon", age=35, children_under_3=1, daycare_expenses_eur=Decimal("1000"))
    )
...
def test_ara16_economia_social():
    profile = ProfileBuilder.from_dict(dict(region="aragon", age=35, ara_economy_social_investment_eur=Decimal("1000")))
    # 20% de 1.000€ = 200€
    _raw = AragonDeductor.calculate_all(profile, Decimal("20000"), Decimal("0"), Decimal("1000"))
    res = _raw
...
def test_ara17_apoyo_refuerzo():
    profile = ProfileBuilder.from_dict(
        dict(region="aragon", age=40, descendants_count=1, ara_school_support_classes_expenses_eur=Decimal("200"))
    )
    # 25% de 200€ = 50€
    _raw = AragonDeductor.calculate_all(profile, Decimal("6000"), Decimal("0"), Decimal("1000"))
    res = _raw
...
def test_ara19_residencia_riesgo_extremo():
    profile = ProfileBuilder.from_dict(dict(region="aragon", age=40, ara_resides_in_extreme_despoblacion=True))
